...
class Course(db.Model):
    ...
    catalog_num = db.Column(db.String(10), nullable=False)
    catalog_num_int = db.Column(db.Integer, nullable=True)
    ...
    def format(self, include_department=False):
        data = {
            ...
            'catalog_num_int': self.catalog_num_int,
            'title': self.title,
            'description': self.description,
            'units': self.units,
            # 'course_code': self.get_course_code(),
            "course_code": f"{self.subject} {self.catalog_num}",
        }
        ...
        if include_department:
            data["department"] = self.department.format() if self.department else None
    ...
    def get_all():
        return db.session.execute(
            db.select(Course).order_by(Course.subject, Course.catalog_num)
        ).scalars().all()
